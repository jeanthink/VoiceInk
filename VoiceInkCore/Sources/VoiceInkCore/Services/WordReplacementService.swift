...
public struct WordReplacementEngine {
    public init() {}
    
    public func applyReplacements(to text: String, using replacements: [WordReplacement]) -> String {
        let enabledReplacements = replacements.filter { $0.isEnabled }
        guard !enabledReplacements.isEmpty else {
            return text
        }
